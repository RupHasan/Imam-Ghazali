function switchLanguage(lang) {
    ...
    const placeholders = document.querySelectorAll('[data-en-placeholder][data-bn-placeholder]');
...
const mobileLangRadios = document.querySelectorAll('input[name="mobile-language"]');
...
document.addEventListener('DOMContentLoaded', function () {
    emailjs.init("dz0sJES_ZdcQ_74OP");
    ...
    if (joinForm) {
        joinForm.addEventListener('submit', function (e) {
            ...
            const templateParams = {
                ...
                phone:     joinForm.querySelector('input[name="phone"]').value || 'Not provided',
                message:   joinForm.querySelector('textarea[name="message"]').value || 'No message provided'
            };

            emailjs.send('service_gobggph', 'template_ym3jfyq', templateParams)
                .then(function () {
                    window.location.href = 'join-us.html';
                })
                .catch(function (error) {
                    alert('❌ There was an issue sending your application. Please try again or contact us directly at hasanrup7@gmail.com');
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function (e) {
            ...
            const templateParams = {
                full_name: contactForm.querySelector('input[name="contact_name"]').value,
                email:     contactForm.querySelector('input[name="contact_email"]').value,
                message:   contactForm.querySelector('textarea[name="contact_message"]').value || 'No message provided'
            };

            emailjs.send('service_gobggph', 'template_2ht25ph', templateParams)
                .then(function () {
                    alert('✅ Message sent! We\'ll get back to you within 2–3 days.');
                    contactForm.reset();
                    submitBtn.textContent = originalText;
                    submitBtn.disabled = false;
                })
                .catch(function (error) {
                    alert('❌ Something went wrong. Please email us directly at hasanrup7@gmail.com');
